...
public sealed class EmployeeProjectsController : ControllerBase
{
    private readonly ISender _sender;

    public EmployeeProjectsController(ISender sender)
    {
        _sender = sender;
    }
    ...
    {
        bool assigned = await _sender.Send(
    ...
    [HttpGet]
    public async Task<ActionResult<IReadOnlyList<EmployeeProjectDto>>> GetAll(
        Guid employeeId,
        CancellationToken cancellationToken)
    {
        IReadOnlyList<EmployeeProjectDto> projects =
            await _sender.Send(
                new GetEmployeeProjectsQuery(employeeId),
                cancellationToken);
    ...
    public async Task<IActionResult> Remove(
        ...
        CancellationToken cancellationToken)
    {
        bool removed = await _sender.Send(
